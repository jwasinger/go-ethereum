...
func (s *BALStateTransition) updateAccount(addr common.Address) (*types.StateAccount, []byte) {
	...
	acct, diff := acct.Copy(), s.diffs.Get(addr)
	code := diff.Code
...
func (s *BALStateTransition) commitAccount(addr common.Address) (*AccountUpdate, *trienode.NodeSet, error) {
	...
	if code := s.diffs.Get(addr).Code; code != nil {
		code := ContractCode{
	...
	if len(s.diffs.Get(addr).StorageWrites) == 0 {
		return op, nil, nil
	}
	...
	for key, value := range s.diffs.Get(addr).StorageWrites {
