...
// returns an empty state root. Used by HistoricDB which does not support
// state hashing.
...
func (h *merkleHasher) UpdateAccount(addresses []common.Address, accounts []AccountMut) error {
	for i, addr := range addresses {
		...
		if acct.Account == nil {
			...
			h.storageRoots[addr] = Hashes{Prev: h.storageRoots[addr].Prev, Hash: types.EmptyRootHash}
			continue
		}
		...
		h.storageRoots[addr] = Hashes{Prev: h.storageRoots[addr].Prev, Hash: storageRoot}
	}
	return nil
}
...
func (h *merkleHasher) Commit() (common.Hash, *trienode.MergedNodeSet, map[common.Address]Hashes, error) {
	...
	root, set := h.accountTrie.Commit(false)
	if set != nil {
		if err := nodes.Merge(set); err != nil {
			return common.Hash{}, nil, nil, err
		}
