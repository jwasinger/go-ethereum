...
// execution of the most recently executed transaction (e.g. via BALANCE /
// EXTCODE* targeting the coinbase, or a value transfer to it), as opposed to
// being touched only by the implicit transaction-fee payment at the end of
// the state transition. The flag is reset at each transaction boundary by
// Prepare and is only meaningful for blocks where access-list construction is
// active (post-Amsterdam).
func (s *StateDB) CoinbaseRead() bool {
	return s.coinbaseRead
}
