...
func (in *Interpreter) Run(snapshot int, contract *Contract, input []byte) (ret []byte, err error) {
	...
	var (
		...
		pcCopy = uint64(0) // needed for the deferred Tracer
		gasCopy = uint64(0) // for Tracer to log gas remaining before execution
		logged = bool(false) // deferred Tracer should ignore already logged steps
	)
