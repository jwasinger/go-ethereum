...
func (f *fieldAllocs) AllocAndSetActive(id uint, modulus []byte, allocSize int) error {
	fieldContext, err := evmmax_arith.NewFieldContext(modulus, allocSize, evmmax_arith.MulModAsm)
	if err != nil {
		return err
	}
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for {
		...
		if operation.dynamicGas != nil {
			...
			dynamicCost, err = operation.dynamicGas(in.evm, callContext, stack, mem, memorySize)
