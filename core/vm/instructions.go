...
func opSetModX(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
    ...
    evmmax_mem_start_stack := scope.Stack.pop()

	mod_offset := mod_offset_stack.Uint64()
	mod_size := mod_limbs_stack.Uint64()
...
func opToMontX(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
    ...
    output_offset_stack := scope.Stack.pop()
    input_offset_stack := scope.Stack.pop()

    output_offset := output_offset_stack.Uint64()
    input_offset := input_offset_stack.Uint64()
