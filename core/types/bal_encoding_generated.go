...
// Hash: ae4b3a65b32e850b3b7570b4389a18798160e05a77500f54787cd04dc12411a4
// Version: 0.1.3
package types
...
func (e *encodingAccountAccess) UnmarshalSSZ(buf []byte) error {
	fmt.Println("unmarshal encoding account access")
	var err error
	size := uint64(len(buf))
	if size < 28 {
		return ssz.ErrSize
...
func (e *encodingAccountBalanceDiff) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Changes'
	{
		...
		for ii := 0; ii < num; ii++ {
			if err = e.Changes[ii].UnmarshalSSZ(buf[ii*40 : (ii+1)*40]); err != nil {
				return err
			}
		}
	}
...
func (e *encodingAccountCodeDiff) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(e.NewCode); size > 24576 {
		err = ssz.ErrBytesLengthFn("encodingAccountCodeDiff.NewCode", size, 24576)
		return
	}
...
func (e *encodingAccountCodeDiff) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'TxIdx'
	e.TxIdx = ssz.UnmarshallUint64(buf[20:28])
